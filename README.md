...
“拾光”（英文名 LastTime）是一个独立的个人周期性事务记录 PWA。记录一件事情最后一次发生的时间，并根据历史记录和用户设置的周期，告诉用户下一次大概什么时候需要关注。
...
- **轻量统计**：记录次数、平均/最长/最短间隔，以及准时率与平均偏差（实际间隔 vs 设定周期，不足两次不显示无意义数据）
- **周期建议**：编辑事项时根据历史间隔中位数推荐周期，一键应用（旧版迁移来的无周期事项受益最大）
- **搜索与排序**：按名称、分类、备注搜索；四种排序（排序方式持久化在链接中，刷新不丢失）
- **数据管理**：JSON 导出 / 校验导入（覆盖模式）、旧“个人工具箱”数据一键迁移
- **分类管理**：默认 8 个分类，可选择图标、增改、拖拽排序和删除（被引用的分类不可删除）
